...
public class Matrice<T> {
    ...
    //Constructor
    public Matrice(Matrice<T> matrice) {
        array = matrice.array;
    }
     ...
     */
    public Matrice(int xSize, int ySize, T defaultValue) {
        this.array = new Object[xSize][ySize];
     ...
     * </ul>
     * @param matrice a matrix with the same type
     * @return a new Matrix<Double> with the result of the addition
     */
    public Matrice<Number> add(Matrice<T> matrice) {
        Matrice<Number> result = new Matrice<>(this.getXSize(), this.getYSize());
        for (int i = 0; i < this.getXSize(); i++) {
            for (int j = 0; j < this.getYSize(); j++) {
                result.setElement(i,j,
                        Double.parseDouble(this.getElement(i,j).toString())+
                                Double.parseDouble(matrice.getElement(i,j).toString())
                );
            }
        }
        return result;
     ...
     */
    public Matrice<Number> mul(Matrice<T> matrice) {
        Matrice<Number> result = new Matrice<>(this.getXSize(), matrice.getYSize());
        for (int i = 0; i < this.getXSize(); i++) {
            for (int j = 0; j < this.getYSize(); j++) {
                double element = 0;
                ...
                for (int k = 0; k < this.getYSize(); k++) {
                    element +=  Double.parseDouble(this.getElement(i,k).toString())*
                                Double.parseDouble(matrice.getElement(k,j).toString());
     ...
     * </ul>
     * @param matrice the Matrix to compare
     * @return if the two matrix are the same
     */
    public boolean equals(Matrice<T> matrice) {
        boolean equal = true;
        if (this.getXSize() != matrice.getXSize()) equal = false;
        if (this.getYSize() != matrice.getYSize()) equal = false;

        int x = 0;
        while (x < this.getXSize() && equal) {
            int y = 0;
            while (y < this.getYSize() && equal) {
                equal = this.getElement(x,y).equals(matrice.getElement(x,y));
                y++;
            }
            x++;
        }
